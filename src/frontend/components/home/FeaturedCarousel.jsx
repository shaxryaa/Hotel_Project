...
const FeaturedCarousel = () => {
  ...
  useEffect(() => {
    fetch('/api/hotels')
      ...
      .then(data => {
        if (data.hotels) {
          // Calculate rating and price for each hotel similar to HotelsPage
          const processed = data.hotels.map(hotel => {
            ...
            return {
              ...
              image: hotel.mainImage || '/hotel1.jpg', // Fallback image
              beds: 2, // Dummy data until model update
              baths: 1, // Dummy data until model update
              sqft: 450 // Dummy data until model update
            };
          });
          setHotels(processed.slice(0, 5)); // Show top 5
        }
      })
  ...
  if (loading) {
    return <div className="text-center py-16">Loading featured hotels...</div>;
  }
  ...
  return (
    <section className="py-16 px-6 bg-gray-50">
      <div className="max-w-7xl mx-auto">
        ...
        {hotels.length === 0 ? (
          <p className="text-center text-gray-500">No hotels available at the moment.</p>
        ) : (
